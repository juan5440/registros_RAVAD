# 📖 Manual de Usuario

Este manual te ayudará a comprender cómo navegar y utilizar todas las funciones del sistema **RAVAD Ledger**.

## 1. Dashboard Principal

Es la pantalla de inicio del sistema. Aquí puedes ver:

- **Tendencias**: Gráfica de ingresos vs egresos anuales.
- **KPIs**: Resumen de saldos totales y estado de aportaciones Pro-Luz del mes actual.
- **Actividad Reciente**: Los últimos 5 movimientos registrados.

## 2. Registro General (Libro Mayor)

Para gestionar tus finanzas diarias:

- **Nuevo Registro**: Haz clic en el botón azul de la barra superior. Llena la fecha, concepto y el monto (Ingreso o Egreso).
- **Búsqueda y Filtros**: Puedes filtrar por texto o por mes/año para encontrar movimientos específicos.
- **Fotos de Facturas**:
  - Haz clic en el icono de la **cámara** para subir una foto de la factura recibida.
  - Haz clic en el icono de la **imagen** para ver la factura ya cargada.

## 3. Módulo Pro-Luz

Gestión específica de aportaciones:

- **Pago de Aportación**: Haz clic en el icono de "pago" (billete) junto al nombre de la persona para registrar su aporte del mes.
- **Sincronización**: Cuando termines de recaudar el mes, usa el botón **Sincronizar** para pasar el total recaudado al Registro General automáticamente.
- **Administrar Personas**: Accede a esta vista para agregar nuevas personas o desactivar las que ya no aportan (esto no borra su historial).

## 4. Reportes y Exportación

El sistema cuenta con tres tipos de reportes avanzados:

- **Detallado de Movimientos**: El listado completo de transacciones con saldos calculados.
- **Mensual de Aportaciones (Pro-Luz)**: Tabla comparativa de quién ha pagado cada mes del año.
- **Resumen Anual**: Totales agregados mes a mes para una visión global del año.

### Cómo Exportar:

En cada pantalla de reporte verás los botones de **Excel** y **PDF**. Al hacer clic, el archivo se descargará automáticamente a tu computadora.

---

> [!TIP]
> **Modo Oscuro**: Puedes cambiar entre modo claro y oscuro usando el icono de la luna/sol en la esquina superior derecha de cualquier página.
